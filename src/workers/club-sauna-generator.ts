...
function shouldCreateClubSauna(date: Date): ClubSaunaConfig {
  ...
  if (
    SHOULDER_SEASON_MONTHS.includes(month) &&
    SHOULDER_SEASON_DAYS.includes(dayOfWeek)
  ) {
    return {
      shouldCreate: true,
...
export async function generateClubSaunas(): Promise<{
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log(
      `[Club Sauna Generator] Creating Club Saunas: ${config.reason}`
    );
    ...
    // eslint-disable-next-line no-console
    console.log(
      `[Club Sauna Generator] Found ${saunas.length} eligible saunas`
    );
    ...
    for (const sauna of saunas) {
      try {
        ...
        if (existing) {
          // eslint-disable-next-line no-console
          console.log(
            `[Club Sauna Generator] Already exists for sauna ${sauna.name}`
          );
          result.skipped++;
          continue;
        }
...
export function scheduleNextRun(): number {
  ...
  // eslint-disable-next-line no-console
  console.log(
    `[Club Sauna Generator] Next run scheduled in ${Math.round(msUntilMidnight / 1000 / 60)} minutes`
  );
