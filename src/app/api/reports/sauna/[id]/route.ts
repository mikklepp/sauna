...
export async function GET(
  ...
  try {
    ...
    const individualAdults = individualReservations.reduce((sum, r) => sum + r.adults, 0);
    const individualKids = individualReservations.reduce((sum, r) => sum + r.kids, 0);
    ...
    // Unique boats
    const individualBoatIds = new Set(individualReservations.map(r => r.boatId));
