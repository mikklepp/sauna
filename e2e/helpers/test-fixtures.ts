...
export async function resetTestClub() {
  ...
  // eslint-disable-next-line no-console
  console.log('  Creating test shared reservations...');
  const now = new Date();
  const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());

  // Upcoming shared reservation (2 hours from now)
  const upcomingStart = new Date(now.getTime() + 2 * 60 * 60 * 1000);
  await prisma.sharedReservation.create({
    data: {
      saunaId: saunas[0].id, // North Main Sauna
      date: today,
      ...
      description: 'Test shared reservation for upcoming events',
      isAutoGenerated: false,
      createdBy: 'e2e_test',
    },
  });

  // Add participants to upcoming reservation
  const upcomingRes = await prisma.sharedReservation.findFirst({
    where: { name: 'Test Upcoming Sauna Event' },
  });
  if (upcomingRes) {
    await prisma.sharedReservationParticipant.createMany({
      data: [
        {
          sharedReservationId: upcomingRes.id,
          boatId: boats[0].id,
          adults: 2,
          kids: 0,
        },
  ...
  // Future shared reservation (tomorrow)
  const tomorrowStart = new Date(today.getTime() + 26 * 60 * 60 * 1000);
  const tomorrow = new Date(today.getTime() + 24 * 60 * 60 * 1000);
  await prisma.sharedReservation.create({
    data: {
      saunaId: saunas[1].id, // North Small Sauna
      date: tomorrow,
      ...
      description: 'Test shared reservation for future events',
      isAutoGenerated: false,
      createdBy: 'e2e_test',
    },
  });

  // Add participant to future reservation
  const futureRes = await prisma.sharedReservation.findFirst({
    where: { name: 'Test Future Sauna Event' },
  });
  if (futureRes) {
    await prisma.sharedReservationParticipant.create({
      data: {
        sharedReservationId: futureRes.id,
        boatId: boats[2].id,
        adults: 3,
        kids: 0,
      },
