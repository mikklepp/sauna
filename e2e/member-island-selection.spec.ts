...
test.describe('Member Island Selection Flow', () => {
  ...
  test('should display list of islands after authentication', async ({ page }) => {
    // Authenticate
  ...
  test('should show island details when clicking on an island', async ({ page }) => {
    // Authenticate
    await page.goto(`/auth?secret=${clubSecret}`);
    await page.waitForURL(/\/islands/, { timeout: 10000 });
    ...
    const islandName = await firstIsland.locator('h3, [class*="CardTitle"]').first().textContent();
    ...
    if (islandName) {
      await expect(page.getByRole('heading', { name: new RegExp(islandName.trim(), 'i') })).toBeVisible();
    }
  ...
  test('should navigate back to islands list from island detail', async ({ page }) => {
    // Authenticate
    await page.goto(`/auth?secret=${clubSecret}`);
    await page.waitForURL(/\/islands/, { timeout: 10000 });
  ...
  test('should display island sauna count correctly', async ({ page }) => {
    ...
    const firstIsland = islandLinks.first();
    const saunaCountText = await firstIsland.locator('[class*="CardDescription"]').first().textContent();
  ...
  test('should allow switching between multiple islands', async ({ page }) => {
    ...
    const firstIslandName = await islandLinks.nth(0).locator('h3, [class*="CardTitle"]').first().textContent();
    const secondIslandName = await islandLinks.nth(1).locator('h3, [class*="CardTitle"]').first().textContent();
    ...
    if (firstIslandName) {
      await expect(page.getByRole('heading', { name: new RegExp(firstIslandName.trim(), 'i') })).toBeVisible();
    }
    ...
    if (secondIslandName) {
      await expect(page.getByRole('heading', { name: new RegExp(secondIslandName.trim(), 'i') })).toBeVisible();
    }
  });

  test('should only show islands belonging to authenticated club', async ({ page }) => {
    ...
    const islandLinks = page.locator('[data-testid="island-link"]');
    const hasIslands = await islandLinks.count() > 0;
    const hasNoMessage = await page.getByText(/no islands available/i).isVisible().catch(() => false);
